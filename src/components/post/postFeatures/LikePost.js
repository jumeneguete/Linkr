...
export default function LikePost({ postDetails, index, arrayOfPosts, setArrayOfPosts }) {
    ...
    const [postLiked, setPostLiked] = useState(
        likes.find(
            (l) => l["user.id"] === userProfile.user.id || l["id"] === userProfile.user.id
        )
    );
    ...
    return (
        <>
            {postLiked ? (
                <IoHeartSharp onClick={handleLikes} color={"#AC0000"} />
            ) : (
                <IoHeartOutline onClick={handleLikes} color={"#FFFFFF"} />
            )}
                        ...
                        ? likes[1]
                            ? likes[2]
                                ? `${
                                      ...
                                      likes[1]["user.username"] || likes[1]["username"]
