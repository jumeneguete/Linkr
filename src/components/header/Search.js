...
export default function Search({ search, setSearch }) {
  ...
  useEffect(() => {
    if (search.length > 2) {
      ...
      const request = axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/users/search?username=${search}`,
        config
      );
  ...
  function suggestFollowersFirst(UsersArray) {
    ...
    UsersArray.forEach((s) => {
      s.isFollowingLoggedUser ? suggetionArray.unshift(s) : suggetionArray.push(s);
    });
  ...
  return (
    <ClickAwayListener onClickAway={() => setSearch("")}>
      <SearchStyle>
        ...
        />
        <SearchSuggestions search={search} usersFound={usersFound} />
      </SearchStyle>
    </ClickAwayListener>
  );
}
